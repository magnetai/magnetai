...
PUBLISH=0
MIRROR=0
CACHEDIR="/Users/qiufan/nosocial/ai-chain/docker"
REBUILD=0
...
#   docker pull nosocial/ai-env:${TOOLCHAIN_VER}
#   if [ $? -ne 0 ]; then
...
#   fi
  ...
  RUN_OPTS="-v $BUILD_DIR:/opt/ai -v $CACHEDIR:/opt/cache"
  ...
  docker run --workdir /opt/ai --cidfile $CIDFILE -i -t --env CARGO_HOME=/opt/cache $RUN_OPTS nosocial/ai-env:${TOOLCHAIN_VER} /bin/bash -c "$CMD"
  CID=`cat $CIDFILE`
  ...
  log_info "ai built at: $DIST_FILE"
}
